...
- "create_employee": { "firstName": string, "lastName": string, "email"?: string, "title"?: string, "department"?: string, "accountRole"?: "owner"|"admin"|"hr"|"clinical_leadership"|"manager"|"staff"|"contractor"|"read_only", "invite"?: boolean } — creates a NEW employee record. Email is OPTIONAL: omit it for a former or contract worker who won't have a login. A login is provisioned only when an email is given and "invite" isn't false. Use this only for people NOT already in the directory (not in the snapshot).
