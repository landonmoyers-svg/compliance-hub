...
1. "completionGuidance": 2–4 short sentences (plain text, no markdown headers) telling the person how to complete THIS form well. Always ground it in documentation best-practices appropriate to the form type: record observable facts not speculation or blame, avoid admissions of liability or legal conclusions, be specific (who/what/when/where), complete it promptly while details are fresh, and note that it becomes a business record. Tailor the emphasis to the form (e.g. incident/root-cause → objective facts + no fault-finding; HR discipline → consistent, factual, non-discriminatory language + the employee's opportunity to respond; PHI authorization → scope/expiration precision). IMPORTANT: if the form references a PATIENT in any way, add a sentence instructing the user to enter a DE-IDENTIFIED reference (patient initials or the last 4 of the MRN) and NOT full patient PHI (name, DOB, full MRN, diagnosis) — this app stores the practice's own compliance records, not PHI, so any completed document containing PHI belongs in the patient's chart/EHR instead. Keep it encouraging and brief. Do NOT give legal advice or cite statutes.
