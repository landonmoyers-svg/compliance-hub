  ...
  },
  {
    ...
    how: [
      ...
      "Document waste/destruction with references.",
          "Labels: one log per campus — M for Murray, L for Lehi. A box is the log, the run of letters, then its letter (M1A, M1B…), and a vial is its box plus a number (M1A1 … M1A10). When a run reaches Z it rolls into the next one (M1Z → M2A), so labels never run out and a vial id is never reused.",
      "FASTEST PATH for a delivery: Receive shipment → photograph the packing slip AND the boxes → Read. It pulls the supplier, PO, NDC, box count and vials per box off the slip, and the serial number, lot and expiry off each box label — including handwriting like \"each box = 10 vials\" or \"1 = A\". Check every number, then Log shipment: it creates the delivery, each box, and a tracked vial inside each one.",
      "If the count it logged doesn't match the paperwork, the delivery is flagged with a count mismatch — resolve it before the vials go into use.",
      "Deliveries lists each shipment with its boxes, how many vials are left in each, and the paperwork itself. Show vials filters the list to one box.",
],
