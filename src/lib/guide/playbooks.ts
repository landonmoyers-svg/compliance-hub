...
export const PLAYBOOKS: Playbook[] = [
  ...
  /* ── added 2026-08-22: the new-company path and the flows added since the
     original set (guided forms + AI review, the PHI checkpoint). ────────── */
  {
    slug: "first-week-owner",
    title: "Your first week as a new practice",
    goal: "Go from an empty account to a working compliance program — people, policies, training, and credentials — in about a week of short sessions.",
    estimate: "~5 sessions",
    roles: ["owner", "admin"],
    steps: [
      {
        ...
        why: "Your practice name, address, NPI and tax ID appear on every exported report and audit packet. Setting them once means every document you generate later is already correct.",
        how: "Fill in the organization profile, then add each physical location you operate. Records can be tied to a site, so add them before you add people.",
        route: "/settings",
        ask: "Walk me through setting up my practice profile and locations.",
      },
      {
        key: "add-people",
        title: "Add your staff",
        why: "Almost everything else attaches to the staff roster — training assignments, credentials, screening, HR documents. Nothing else works well until the roster is real.",
        how: "Add each person with their name, email, department, and job role. Add everyone first; you can invite their logins afterwards.",
        route: "/hr/employees", anchor: "add-employee",
        ask: "Help me add my staff.",
      },
      {
        key: "invite-logins",
        title: "Give people access at the right role",
        why: "Role is the access boundary — it decides who can see payroll, discipline, and sensitive HR files. Grant the least each person needs.",
        how: "Work the 'No account yet' tile. Invite each person; they set their own password from the email. Choose the role deliberately.",
        route: "/user-management",
      },
      {
        key: "load-policies",
        title: "Load your policies and SOPs",
        why: "Policies are what your training, attestations, and Policy Q&A all reference. Without them the program has nothing to point at.",
        how: "Drop your existing documents in — a whole folder or a .zip is fastest, and the AI files each one to the right module. Review each destination before filing.",
        route: "/document-intake",
        ask: "How should I bring in my existing policy documents?",
      },
      {
        key: "credentials",
        title: "Get every licence and certificate on file",
        why: "An expired licence on a billing provider is the single most expensive compliance failure a practice can have. Tracking expiry is the whole point.",
        how: "Add each clinician's licences, DEA registrations, board certs and malpractice cover. Bulk Upload takes a pile of certificates at once.",
        route: "/credentials", anchor: "add-credential",
      },
      {
        key: "assign-training",
        title: "Assign the required training",
        why: "HIPAA requires workforce training and OSHA requires it for the hazards your staff face. A dated completion record is the evidence.",
        how: "Use 'Assign to all staff' for the things everyone needs — HIPAA, harassment, bloodborne pathogens — rather than assigning one by one.",
        route: "/training",
      },
      {
        key: "screen-everyone",
        title: "Screen everyone against the exclusion lists",
        why: "Billing a federal program for anything involving an excluded person triggers penalties. Screen at hire and monthly after.",
        how: "Run automated screening — it checks the whole roster against the official OIG-LEIE at once — then download the evidence certificate for your records.",
        route: "/exclusion-screening", anchor: "run-screening",
      },
      {
        key: "daily-habit",
        title: "Set the habit that keeps it working",
        why: "Compliance fails by drift. Ten minutes a day on the queue prevents nearly every finding.",
        how: "Open Home each morning and clear Overdue and Due today. Then use Plan my week in the Guide so deadlines reach you before they bite.",
        route: "/",
      },
    ],
  },
  {
    slug: "add-a-company",
    title: "Add another company (platform)",
    goal: "Stand up a separate practice with its own owner, staff, and completely isolated data.",
    estimate: "~5 min",
    roles: ["owner"],
    steps: [
      {
        key: "understand-isolation",
        title: "Know what 'separate' means here",
        why: "Companies share the software, never the data. Nobody in one company can see another's staff, credentials, or documents — and a role only applies inside the company it was granted in.",
        how: "Read the Companies, roles & sites lesson so you can explain the boundary to the new owner with confidence.",
        route: "/guide",
        ask: "Explain how company and site isolation works.",
      },
      {
        key: "create-company",
        title: "Create the company and invite its owner",
        why: "The first owner is who runs that practice. They get their own account and administer their own people — they cannot reach yours.",
        how: "Settings → Organization → Companies. Enter the practice name plus the owner's name and email. They receive an invite to set their own password.",
        route: "/settings",
      },
      {
        key: "hand-off",
        title: "Hand them the first-week path",
        why: "A brand-new company starts empty — no policies, forms, training modules, or regulatory sources. Left alone that feels broken rather than clean.",
        how: "Point their owner at the 'Your first week as a new practice' playbook, which walks them from empty to operational.",
        route: "/guide",
      },
    ],
  },
  {
    slug: "complete-a-form",
    title: "Complete a form properly",
    goal: "Fill in a compliance form so it is accurate, complete, and defensible if anyone reads it later.",
    estimate: "~5 min",
    steps: [
      {
        key: "open-it",
        title: "Open the form and read the guidance",
        why: "How a record is worded decides whether it protects you or weakens you. The blue panel at the top tells you what 'done well' looks like for that specific form.",
        how: "Forms → click Fill out on any active template. You do not need it assigned to you first. Read the 'How to complete this properly' panel, and the one-line hint under each field.",
        route: "/fillable-documents",
      },
      {
        key: "prefill",
        title: "Let AI fill what it already knows",
        why: "Retyping details the app already holds wastes time and introduces typos.",
        how: "Click AI prefill. It fills from the employee and organization records and labels each value with its source — check each one before moving on.",
        route: "/fillable-documents",
      },
      {
        key: "phi-check",
        title: "Keep patient information out of it",
        why: "This app holds the practice's own records, not patient data. A completed document containing PHI belongs in the chart, not here.",
        how: "Use a de-identified reference — initials or the last 4 of the MRN. When asked 'Does this contain patient information?', answer honestly; saying yes gives you a local copy for the chart instead of saving it here.",
        route: "/fillable-documents",
        ask: "What counts as patient information I should not put in this app?",
      },
      {
        key: "review",
        title: "Have the AI check your wording before you submit",
        why: "Speculation, blame, or an admission of liability in a business record can be used against the practice later. Catching it before submission costs nothing.",
        how: "Click Review answers. It flags anything speculative or blaming and suggests defensible wording. Editing a flagged field clears its warning; then submit.",
        route: "/fillable-documents",
      },
    ],
  },
  {
    ...
    title: "Report an incident correctly",
    goal: "Get an event on the record factually, promptly, and without putting patient information where it does not belong.",
    estimate: "~10 min",
    steps: [
      {
        key: "report",
        title: "Report it while it is fresh",
        why: "Contemporaneous records carry weight; reconstructed ones invite doubt. Report the day it happens.",
        how: "Choose the report type that fits, then describe only what you personally observed or were directly told. No speculation about why or who is at fault.",
        route: "/incidents", anchor: "add-incident",
      },
      {
        key: "phi-checkpoint",
        title: "Answer the patient-information question honestly",
        why: "An incident naming a patient is PHI and must not be stored here — it belongs in the chart.",
        how: "If it contains patient information, say so: the app blocks the save and generates a local copy on your device for the chart. Then log a de-identified version here (initials or MRN last 4) so the compliance record still exists.",
        route: "/incidents",
      },
      {
        key: "breach-check",
        title: "If PHI was involved, run the four-factor assessment",
        why: "HIPAA presumes a breach unless a documented four-factor risk assessment shows low probability of compromise. That assessment is the record that decides whether you must notify.",
        how: "Work the four factors honestly and let the tool derive the conclusion.",
        route: "/breach-assessment", anchor: "four-factor",
      },
      {
        key: "capa",
        title: "Close the loop with a corrective action",
        why: "An incident with no corrective action is an unfixed problem. Auditors look for the follow-through, not just the report.",
        how: "Add a corrective action with an owner and a due date, then track it to completion.",
        route: "/incidents", anchor: "add-capa",
      },
    ],
  },
