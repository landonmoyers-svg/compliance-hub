...
export function buildSdsSheetHtml(r: SDSRecord): string {
  ...
  const hazardBody = `
    <table class="kv">
      <tr><th>Signal word</th><td>${r.signalWord && r.signalWord !== "NONE" ? `<span class="signal ${r.signalWord.toLowerCase()}">${esc(r.signalWord)}</span>` : '<span class="muted">None</span>'}</td></tr>
    </table>
...
<title>SDS — ${esc(r.productName)}</title>
<style>
...
<body>
  ...
  <header>
    ...
    <div class="meta">${esc(r.manufacturer || "")}${r.casNumber ? ` · CAS ${esc(r.casNumber)}` : ""} · SDS revision ${esc(r.revisionDate || "not specified")} · Generated ${esc(now.toLocaleDateString())}</div>
  </header>
